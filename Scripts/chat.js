    ...
    currentRequest = $.ajax({
      ...
      success: function (response) {
        ...
        if (
          ...
          $("#" + uniqueID).html(
            "<small class='text-warning'>⚠️ No reply received from chatbot.</small>"
          );
          return;
        }

        const escapedResponse = $("<div>").text(response.response).html();
        typeWriterEffect($("#" + uniqueID)[0], escapedResponse, 40);
      },
      ...
      error: function (xhr, status, error) {
        ...
        $("#" + uniqueID).html(
          "<small class='text-danger'>❌ Error fetching response.</small>"
        );
      },
      complete: function () {
        currentRequest = null;
...
function typeWriterEffect(element, text, speed = 50, callback = null) {
  if (!text) {
    element.innerHTML =
      "<small class='text-danger'>❌ No text to display.</small>";
    return;
  }
